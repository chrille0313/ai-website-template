...
export const corsHeaders: Record<string, string> = {
  ...
  'Access-Control-Allow-Headers':
    `${supabaseCorsHeaders['Access-Control-Allow-Headers']}, user-agent, sentry-trace, baggage`
}
...
type AuthRouteHandler<T> = (ctx: T, req: Request, url: URL, params: Params) => Response | Promise<Response>
 ...
 */
export function createAuthRouter<T>(functionName: string, contextFactory: (req: Request) => Promise<T | Response>) {
  const { addRoute, dispatch } = buildRouterCore(functionName)
